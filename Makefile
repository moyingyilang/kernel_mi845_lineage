...
export CPP AR NM STRIP OBJCOPY OBJDUMP DIS
...
# Tell clang to never replace conditional load with a non-conditional one
...
tools/: FORCE
	...
	$(Q)$(MAKE) LDFLAGS= MAKEFLAGS="$(tools_silent) $(filter --j% -j,$(MAKEFLAGS))" O=$(shell cd $(objtree) && /bin/pwd) subdir=tools -C $(srctree)/tools/
